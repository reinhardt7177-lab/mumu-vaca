...
async function getClassStudentsInternal({ classId, classCode, teacherUid }) {
  const db = getFirestoreClient();
  const studentsQuery = query(collection(db, "profiles"), where("classCode", "==", classCode));
  ...
  return studentsSnap.docs
    ...
    .filter((item) => {
      ...
      return String(item.teacherUid ?? "") === String(teacherUid);
    })
    .sort((a, b) => toMillis(b.createdAt) - toMillis(a.createdAt));
}

async function getClassInvitesInternal({ classId }) {
  const db = getFirestoreClient();
  const invitesQuery = query(collection(db, "student_invites"), where("classId", "==", classId));
...
export async function listTeacherClasses() {
  ...
  const classes = await Promise.all(
    classesSnap.docs.map(async (item) => {
      ...
      const students = await getClassStudentsInternal({ classId, classCode, teacherUid: user.uid });
      const invites = await getClassInvitesInternal({ classId });
...
export async function getTeacherClassBoard(classId) {
  ...
  const [students, invites] = await Promise.all([
    getClassStudentsInternal({ classId: classInfo.classId, classCode: classInfo.classCode, teacherUid: user.uid }),
    getClassInvitesInternal({ classId: classInfo.classId })
  ]);
