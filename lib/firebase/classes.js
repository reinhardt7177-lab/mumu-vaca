...
async function getClassStudentsInternal({ classId, classCode, teacherUid }) {
  const db = getFirestoreClient();
  const studentsQuery = query(collection(db, "profiles"), where("teacherUid", "==", teacherUid));
...
export async function listTeacherClasses() {
  ...
  const classes = await Promise.all(
    classesSnap.docs.map(async (item) => {
      ...
      const invites = await getClassInvitesInternal({ classId, teacherUid: user.uid });
...
export async function getTeacherClassBoard(classId) {
  ...
  const [students, invites] = await Promise.all([
    ...
    getClassInvitesInternal({ classId: classInfo.classId, teacherUid: user.uid })
  ]);
